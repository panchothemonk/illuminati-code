...
async function askForApiKey(): Promise<boolean> {
  ...
  const key = await new Promise<string>((resolve) => {
    askRl.question('\x1b[32mEnter your Kimi API key: \x1b[0m', (answer) => {
      resolve(answer.trim())
    })
    askRl.on('SIGINT', () => {
      askRl.close()
  ...
  try {
    ...
    if (text === 'exit' || text === 'quit') {
      if (activeCoordinator) {
        activeCoordinator.cancelAll('User exited')
    ...
    console.log(`\x1b[31mError: ${handlerErr.message}\x1b[0m`)
    if (DEBUG) console.log(handlerErr.stack || '')
  } finally {
    rl.prompt()
  }
})
...
for (const signal of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
  process.on(signal, () => {
    ...
    if (activeCoordinator) {
      activeCoordinator.cancelAll('Process interrupted')
      activeCoordinator = undefined
    }
    sessionManager.destroy()
    rl.close()
...
process.on('unhandledRejection', (reason, promise) => {
  console.error('\x1b[31m[Unhandled Rejection]', reason, '\x1b[0m')
})
...
async function handleChat(text: string): Promise<void> {
  ...
  while (msgQueue.length > 0) {
    ...
    try {
      ...
      console.log(`\x1b[31mError processing queued message: ${queueErr.message}\x1b[0m`)
    }
  }
