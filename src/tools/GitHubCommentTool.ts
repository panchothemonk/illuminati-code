...
export const GitHubCommentTool: Tool = {
  ...
  async execute(args) {
    ...
    try {
      ...
      if (hasGh) {
        const cmd = ['gh', 'issue', 'comment', String(args.issueNumber), '--repo', repo, '--body', args.body]
        const { stdout, stderr, exitCode } = await $(cmd)
        const out = stdout.toString().trim()
        const err = stderr.toString().trim()
        if (exitCode !== 0) {
          // Try PR comment if issue comment failed
          const prCmd = ['gh', 'pr', 'comment', String(args.issueNumber), '--repo', repo, '--body', args.body]
          const prResult = await $(prCmd)
